...
const ProgressRing: React.FC<Props> = ({ radius, stroke, time }) => {
  ...
  return (
    <svg height={radius * 2} width={radius * 2}>
      <circle stroke="white" fill="transparent" strokeWidth={stroke} strokeDasharray={circumference + " " + circumference} style={{ strokeDashoffset }} r={normalisedRadius} cx={radius} cy={radius} />
    </svg>
  );
};
